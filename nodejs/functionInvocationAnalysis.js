...
(function (sandbox) {
	...
	// open log file
	const logPath = J$.initParams.log;
	const logFd = fs.openSync(logPath, 'w');
        
    // store names of (some) core modules
    const supportedModules = require('./supportedModules.js')
	...
	function getObjId(val){
	    ...
	    metadata.argIds=argIds;
            return instr.before(metadata,logFd);
        }
        ...
        function afterFunction(metadata) {
            ...
            return instr.after(metadata,logFd);
        }
